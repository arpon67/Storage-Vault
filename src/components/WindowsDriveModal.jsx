...
export function WindowsDriveModal() {
  ...
  const buildBat = (letter, name) => {
    ...
    return [
      ...
      `)`,
      ``,
      `reg add "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\DriveIcons\\${letterClean}\\DefaultLabel" /ve /d "${safeName} Unlimited Vault - ${letter}" /f >nul 2>&1`,
      ``,
      `set NET_SHORTCUT=%APPDATA%\\Microsoft\\Windows\\Network Shortcuts\\${safeName}_${letterClean}`,
  ...
  const buildPs1 = (letter, name) => {
    ...
    return [
      ...
      `try { net use ${letter} /delete /yes 2>$null } catch {}`,
      ``,
      `try {`,
      `    New-PSDrive -Name $LetterClean -PSProvider FileSystem -Root "http://127.0.0.1:8080/vault" -Persist -Scope Global -ErrorAction SilentlyContinue | Out-Null`,
      `} catch {}`,
      ``,
      `$NetShortcut = Join-Path $env:APPDATA "Microsoft\\Windows\\Network Shortcuts\\${safeName}_${letterClean}"`,
      `if (!(Test-Path $NetShortcut)) { New-Item -ItemType Directory -Path $NetShortcut -Force | Out-Null }`,
      `$ws = New-Object -ComObject WScript.Shell`,
      `$sc = $ws.CreateShortcut((Join-Path $NetShortcut "target.lnk"))`,
      `$sc.TargetPath = "http://127.0.0.1:8080/vault"`,
      `$sc.Save()`,
      ``,
      `Write-Host "SUCCESS: ${safeName} (${letter}) Cloud Storage Vault active!" -ForegroundColor Green`,
      `Invoke-Item $NetShortcut`,
    ].join('\r\n');
  };
